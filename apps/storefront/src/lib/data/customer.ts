...
async function completeLogin(
  ...
  if (typeof result === "object" && "location" in result) {
    return {
      state: "error",
      error: "Metode masuk ini tidak didukung oleh storefront.",
    }
  }
  ...
  if (typeof result !== "string") {
    return {
      state: "error",
      error: "Autentikasi memerlukan langkah tambahan yang tidak didukung.",
    }
  }
...
export const updateCustomerAddress = async (
  ...
  if (!addressId) {
    return { success: false, error: "ID alamat wajib diisi" }
  }
